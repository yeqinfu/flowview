...
/**
 * Created by yeqinfu on 2019/5/5
 */
public class DraggableFrameLayout extends FrameLayout {

    private ViewDragHelper dragHelper;
    private int screenWidth;
    private int statusType = 2;//0无 随便移动   1靠左  2靠右 0靠左右
    ...
    public DraggableFrameLayout(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        ...
        statusType = typedArray.getInt(R.styleable.DraggableFrameLayout_direction, 2);
    ...
    private void init() {
        ...
        dragHelper = ViewDragHelper.create(this, new ViewDragHelper.Callback() {
            ...
            public void onViewReleased(View releasedChild, float xvel, float yvel) {
                ...
                curRight=releasedChild.getRight();
                curBottom=releasedChild.getBottom();
                Log.d("yeqinfu", "===" + curLeft + "==" + curTop + "===" + curRight + "==" + curBottom);
                ...
                Log.d("yeqinfu","======"+finalLeft+"========"+finalTop);
                invalidate();
            }
        });
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        Log.d("yeqinfu","===========onLayout========");
        if (textChange) {
            textChange = false;

            super.onLayout(changed, left, top, right, bottom);

            //findViewById(R.id.tv_msg).requestLayout();
            getChildAt(0).layout(curLeft,curTop,curRight,curBottom);
        } else if (finalLeft == -1 && finalTop == -1) {
            super.onLayout(changed, left, top, right, bottom);
        }
